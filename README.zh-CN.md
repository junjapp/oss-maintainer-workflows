...
1. 先只保留你的仓库真的会用到的 issue 模板和公开说明。
2. 把所有占位维护者信息替换成真实姓名、团队或归属规则。
3. 把首个版本当作结构校验，而不是功能大版本。
4. 只有在基础 intake、review、release 节奏稳定后，再逐步加入更多自动化。

## 仓库边界

- 这个仓库应该主要描述维护者如何做 intake、review、release 和 ownership 相关工作。
- 它不应该膨胀成产品脚手架、内部运营后台，或杂乱的自动化集合。
- 如果某个文件不能被外部维护者用几句话解释清楚，它大概率就不应该放在这里。
